...
// ⚠️ LS증권 API 는 Worker 에서 호출하지 않는다 (LS 등록 IP = 로컬 Windows 프로세스).
//    LS 토큰/시세/잔고/주문은 local-runner/ 에서만 실행한다. 이 파일에 LS import 금지.
...
// (LS 잔고 점검은 Worker 가 아니라 local-runner 에서: `npm run ls:diag`)
