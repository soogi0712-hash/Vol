...
import { maskAccount, makeScrubber } from './mask';
...
export async function runBalanceCheck(cfg: LocalLSConfig, log: Logger): Promise<BalanceReport> {
  ...
  const report: BalanceReport = {
    ...
    kr_balance_ok: false, kr_total_eval: null, kr_orderable_cash: null,
    us_balance_ok: false, us_total_eval_krw: null,
  ...
  try {
    ...
    log.info(`KR 잔고 OK — 총평가=${b.totalEval.toLocaleString()} 주문가능현금=${b.orderableCash.toLocaleString()}`);
  } catch (e) {
  ...
  try {
    ...
    log.info(`US 잔고 OK — 원화환산 총평가=${b.totalEvalKRW.toLocaleString()}`);
  } catch (e) {
