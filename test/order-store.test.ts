...
describe('일일 매수 한도 + 동일봉 중복주문 방지', () => {
  it('하루 매수 1회 제한', () => {
    ...
    s.recordPlaced('buy', '20260706093000', '20260706', { ordNo: '1', symbol: 'AAPL', qty: 1, price: 100, placedAtMs: 1 });
...
describe('미체결 추적 + 재시작 중복주문 방지', () => {
  it('pending 추가/해소', () => {
    ...
    s.recordPlaced('buy', '20260706093000', '20260706', { ordNo: '141', symbol: 'AAPL', qty: 1, price: 100, placedAtMs: 1 });
  ...
  it('flush 후 새 인스턴스 load 시 일일카운트·주문봉·pending 복원(재시작 중복주문 방지)', () => {
    ...
    s1.recordPlaced('buy', '20260706093000', '20260706', { ordNo: '141', symbol: 'AAPL', qty: 1, price: 100, placedAtMs: 1 });
    s1.flush();
    ...
    expect(s2.pending[0].ordNo).toBe('141');                          // 주문번호 복원(req16)
    expect(s2.pending[0].placedAtMs).toBe(1);                         // 주문시각 복원(타임아웃 판정)
  });
});

describe('LS 원문 rsp_cd/rsp_msg 감사(req18)', () => {
  it('recordResponse 저장 + 재시작 복원', () => {
    const s1 = new OrderStore('AAPL', dir);
    s1.recordResponse({ atMs: 1, tr: 'COSAT00301', rspCd: '00000', rspMsg: '정상', ordNo: '141' });
    s1.recordResponse({ atMs: 2, tr: 'COSAT00311', rspCd: 'IGW00XX', rspMsg: '거부', ordNo: '141' });
    s1.flush();
    const s2 = new OrderStore('AAPL', dir); s2.load();
    expect(s2.responses).toHaveLength(2);
    expect(s2.responses.map(r => r.tr)).toEqual(['COSAT00301', 'COSAT00311']);
    expect(s2.responses[1].rspCd).toBe('IGW00XX');
  });
});
